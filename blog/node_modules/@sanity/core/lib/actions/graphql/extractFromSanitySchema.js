...
function isBaseType(type) {
  return type.name !== type.jsonType && allowedJsonTypes.includes(type.jsonType) && !skipTypes.includes(type.name);
}
...
function extractFromSanitySchema(sanitySchema) {
  ...
  function getArrayChildDefinition(child, arrayDef) {
    ...
    return isReference(child) ? getReferenceDefinition(child, arrayDef) : {
  ...
  function gatherAllFields(type) {
    let field = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'fields';

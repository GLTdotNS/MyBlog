...
var _isNpx = require("./isNpx");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const commandPrefix = (0, _isNpx.prefixCommand)();
/**
...
function generateCommandsDocumentation(commandGroups, group = 'default') {
  ...
  const rows = [`usage: ${commandPrefix}${prefix} [--default] [-v|--version] [-d|--debug] [-h|--help] <command> [<args>]`, '', 'Commands:'].concat(commands.map(cmd => `   ${(0, _lodash.padEnd)(cmd.name, cmdLength + 1)} ${cmd.description}`)).concat(['', `See '${commandPrefix} help${prefix} <command>' for specific information on a subcommand.`]);
  return rows.join('\n');
}
/**
...
function generateCommandDocumentation(command, group, subCommand) {
  ...
  return [`usage: ${commandPrefix} ${cmdParts} ${command.signature}`, '', `   ${command.description}`, '', (command.helpText || '').trim()].join('\n');
}